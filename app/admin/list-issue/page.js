...
const handleSort = (field) => {
  ...
  const fetchAllIssuesForExport = async () => {
  const token = Cookies.get('token');
  try {
    const res = await axios.post(
      'http://127.0.0.1:8000/api/get-transaction-all',
      {
        ...
        pageSize: 10000, // ambil semua data
        pageNumber: 0,
...
const exportIssuesToExcel = async () => {
  const data = await fetchAllIssuesForExport(); // ambil semua data sesuai filter
  ...
  worksheet.columns = [
    ...
    { header: 'Last Modified', key: 'created_at', width: 25 },
  ];
  ...
  return (
    <div className={styles.container}>
      <div className="flex flex-wrap gap-4 items-center justify-between bg-white rounded-xl shadow-md px-8 py-5 mb-6 mt-6 w-full">
        ...
        {/* Export Excel Button - Positioned to the right */}
              ...
              issues.map((issue, index) => (
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <button className={styles.actionButton} onClick={() => router.push(`/admin/edit-issue?id=${issue.id}&issueid=${issue.issueid}`)}><Pencil size={19}/></button>
                  </td>
                </tr>
              ))
            ) : (
